...
def get_droplets():
    ...
    response = {"success": True, "droplets": []}
...
def request_new_instance():
    try:
        ...
        if not Permissions.user_in_groups(current_user.id, droplet.allowed_groups):
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "You do not have access to this droplet",
                    }
                ),
                403,
            )
        ...
        if not utils.docker.docker_client:
            ...
            return (
                jsonify({"success": False, "error": "Docker service is not available"}),
                500,
            )
        ...
        try:
            ...
            return (
                jsonify(
                    {
                        "success": False,
                        "error": f"Failed to communicate with Docker: {str(e)}",
                    }
                ),
                500,
            )
        ...
        if (
            ...
            and "docker.io" not in droplet.container_docker_registry
        ):
        ...
        for image in images:
            if (
                isGuacDroplet
                and f"flowcaseweb/flowcase-guac:{__version__}" in image.tags
            ):
                image_exists = True
                break
        ...
        if not image_exists:
            log(
                "WARNING",
                f"Docker image {droplet.container_docker_image} not found. Please wait a few minutes and try again.",
            )
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "Docker image not found. Image might still be downloading.",
                    }
                ),
                400,
            )
        ...
        log(
            "INFO",
            f"Creating new instance for user {current_user.username} with droplet {droplet.display_name}",
        )
        ...
        if (
            request_resolution
            and len(request_resolution) < 10
            and re.match(r"[0-9]+x[0-9]+", request_resolution)
        ):
        ...
        if (
            ...
            try:
                ...
                if not profilePath.exists():
                    raise RuntimeError(
                        f"Failed to create profile directory: {profilePath}"
                    )

                if not profilePath.is_dir():
                    raise RuntimeError(
                        f"Profile path exists but is not a directory: {profilePath}"
                    )

                # Check if directory is writable
                if not os.access(str(profilePath), os.W_OK):
                    raise RuntimeError(
                        f"Profile directory exists but is not writable: {profilePath}"
                    )

                log(
                    "INFO",
                    f"Successfully created and verified profile directory: {profilePath}",
                )
        ...
        try:
            ...
            if not isGuacDroplet:
                ...
                # If we are using a custom network, we ensure the Nginx proxy is connected to it
                # so it can reach this container without breaking isolation.
                if network_name != "flowcase_default_network":
                    try:
                        nginx_container = utils.docker.docker_client.containers.get(
                            "flowcase-nginx"
                        )
                        # Refresh container info to get latest networks
                        nginx_container.reload()
                        nginx_networks = nginx_container.attrs["NetworkSettings"][
                            "Networks"
                        ]

                        if network_name not in nginx_networks:
                            custom_net = utils.docker.docker_client.networks.list(
                                names=[network_name]
                            )
                            if custom_net:
                                custom_net[0].connect(nginx_container)
                                log(
                                    "INFO",
                                    f"Connected flowcase-nginx to {network_name} for access to {name}",
                                )
                            else:
                                log(
                                    "WARNING",
                                    f"Network {network_name} not found! Nginx might not reach container.",
                                )
                    except Exception as e:
                        log(
                            "ERROR",
                            f"Failed to connect flowcase-nginx to {network_name}: {e}",
                        )
            ...
            log(
                "INFO",
                f"Instance created for user {current_user.username} with droplet {droplet.display_name}",
            )
            ...
            while waited_time < max_wait_time:
                ...
                try:
                    ...
                    if container.status == "running":
                        log(
                            "INFO",
                            f"Container {name} is running after {waited_time} seconds",
                        )
                        break
                    elif container.status in ["exited", "dead"]:
                        log(
                            "ERROR",
                            f"Container {name} failed to start, status: {container.status}",
                        )
            ...
            if waited_time >= max_wait_time:
                log(
                    "ERROR",
                    f"Container {name} startup timed out after {max_wait_time} seconds",
                )
                try:
                ...
                db.session.commit()
                return (
                    jsonify({"success": False, "error": "Container startup timed out"}),
                    500,
                )
            ...
            try:
                ...
                if not ip:
                    log(
                        "ERROR",
                        f"Could not find IP address for container {name} on flowcase_default_network",
                    )
                    container.remove(force=True)
                    db.session.delete(instance)
                    db.session.commit()
                ...
                return (
                    jsonify(
                        {
                            "success": False,
                            "error": "Failed to get container network information",
                        }
                    ),
                    500,
                )
            ...
            log(
                "ERROR",
                f"Error creating container for user {current_user.username}: {str(e)}",
            )
            # Cleanup on failure
            try:
            ...
            return (
                jsonify(
                    {"success": False, "error": f"Failed to create container: {str(e)}"}
                ),
                500,
            )
...
def check_resources(droplet: Droplet) -> Tuple[bool, str]:
    ...
    droplets = (
        Droplet.query.filter(Droplet.id.in_(droplet_ids)).all() if droplet_ids else []
    )
    ...
    for instance in instances:
        ...
        if instance_droplet:
            ...
            try:
                ...
                # Container not found or error accessing it - assume not consuming resources
                pass
    ...
    if projected_memory_usage > max_allowed_memory:
        log(
            "ERROR",
            f"Insufficient memory for user {current_user.username} to request droplet {droplet.display_name} - would use {projected_memory_usage}MB of {max_allowed_memory}MB allowed",
        )
    ...
    if projected_core_usage > max_allowed_cores:
        log(
            "ERROR",
            f"Insufficient CPU cores for user {current_user.username} to request droplet {droplet.display_name} - would use {projected_core_usage} of {max_allowed_cores} cores allowed",
        )
...
def generate_nginx_config(
    instance: DropletInstance, droplet: Droplet, ip: str, user: User
) -> str:
    """Generate nginx configuration for the instance."""
    try:
        authHeader = base64.b64encode(
            b"flowcase_user:" + user.auth_token.encode()
        ).decode("utf-8")
        ...
        log(
            "INFO",
            f"Generated nginx config for instance {instance.id}: IP={ip}, type={droplet.droplet_type}",
        )
        return nginx_config
...
def pull_droplet_image(droplet_id):
    ...
    if not droplet.container_docker_image:
        return (
            jsonify(
                {"success": False, "error": "Droplet has no Docker image configured"}
            ),
            400,
        )
    ...
    try:
        ...
        success, message = utils.docker.pull_single_image(
            droplet.container_docker_registry, droplet.container_docker_image
        )
        ...
        return (
            jsonify({"success": False, "error": f"Failed to pull image: {str(e)}"}),
            500,
        )
