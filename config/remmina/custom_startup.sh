...
kasm_exec() {
    ...
    if [ -n "$URL" ] ; then
        # /usr/bin/desktop_ready check replacement
        if [ -z ${START_DE+x} ]; then   START_DE="xfce4-session"; fi
        while ! ps aux | grep -v grep | grep ${START_DE} > /dev/null; do
            sleep 0.5
        done
    ...
    if [ -z "$DISABLE_CUSTOM_STARTUP" ] ||  [ -n "$FORCE" ] ; then
        ...
        do
            ...
            then
                # /usr/bin/desktop_ready check replacement
                if [ -z ${START_DE+x} ]; then   START_DE="xfce4-session"; fi
                # Loop until xfce4-session is found
                while ! ps aux | grep -v grep | grep ${START_DE} > /dev/null; do
                    sleep 0.5
                done
